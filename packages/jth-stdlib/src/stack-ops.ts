...
export const clear = (stack: Stack) => {
  stack.clear();
};
...
export const keepN = (n: number) => (stack: Stack) => {
...
export const keepHalf = (stack: Stack) => {
  const arr = stack.toArray();
  stack.clear();
...
export const dropHalf = (stack: Stack) => {
  const arr = stack.toArray();
  stack.clear();
...
export const copy = (stack: Stack) => {
  const arr = stack.toArray();
  stack.push(...arr);
};

// dupe: duplicate top item
export const dupe = (stack: Stack) => {
  stack.dup();
};

// retrieve: configured -- retrieve(0) gets top, retrieve(1) gets second from top
export const retrieve = (index = 0) => (stack: Stack) => {
  const arr = stack.toArray();
...
export const dig = (n = 1) => (stack: Stack) => {
  const arr = stack.toArray();
...
export const bury = (n = 1) => (stack: Stack) => {
  const arr = stack.toArray();
  if (arr.length < 2) return;
  const idx = n < 0 ? ~n : arr.length + ~n;
  if (idx >= 0 && idx < arr.length) {
...
export const cycle = (n = 1) => (stack: Stack) => {
  const arr = stack.toArray();
...
export const recycle = (n = 1) => (stack: Stack) => {
  const arr = stack.toArray();
...
export const hold = (f: unknown) => (stack: Stack) => {
  stack.push(f);
};
...
export const reverse = (stack: Stack) => {
...
export const count = (stack: Stack) => {
