...
export function parse(tokens: Token[]): ProgramNodeType {
  ...
  function parseProgram(): ProgramNodeType {
    const body: any[] = [];
  ...
  function parseExpression(): any {
    ...
    switch (tok.type) {
      case TokenType.NUMBER: {
        advance();
        return AST.NumberLiteral(tok.value as number, tok.line, tok.column);
      }
      case TokenType.STRING: {
        advance();
        return AST.StringLiteral(tok.value as string, tok.line, tok.column);
      }
      case TokenType.BOOLEAN: {
        advance();
        return AST.BooleanLiteral(tok.value as boolean, tok.line, tok.column);
      }
      case TokenType.NULL: {
        advance();
      ...
      case TokenType.OPERATOR: {
        advance();
        return AST.OperatorCallNode(tok.value as string, [], tok.line, tok.column);
      }
      ...
      case TokenType.INLINE_JS: {
        advance();
        return AST.InlineJSExpression(tok.value as string, tok.line, tok.column);
      }
      case TokenType.DEFINITION: {
        advance();
        return AST.DefinitionNode(tok.value as string, tok.line, tok.column);
      }
      case TokenType.VALUE_DEFINITION: {
        advance();
        return AST.ValueDefinitionNode(tok.value as string, tok.line, tok.column);
      }
  ...
  function parseIdentifierOrConfiguredOp(): any {
    ...
    if (current().type === TokenType.PAREN_OPEN) {
      ...
      advance(); // skip )
      return AST.OperatorCallNode(tok.value as string, args, tok.line, tok.column);
    }
    ...
    if (current().type === TokenType.OBJECT_OPEN) {
      ...
      while (current().type !== TokenType.OBJECT_CLOSE) {
        ...
        if (current().type === TokenType.IDENTIFIER) {
          bindings.push(advance().value as string);
        } else {
          throw new JthParserError(
  ...
  function parseExport(): any {
    ...
    while (
      ...
      if (current().type === TokenType.IDENTIFIER) {
        names.push(advance().value as string);
      } else {
        throw new JthParserError(
