...
export function processN(stack: Stack, items: Iterable<unknown>): Stack | Promise<Stack> {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (result && typeof (result as Promise<unknown>).then === "function") {
      return processNAsync(stack, arr, i, result as Promise<unknown>, saved, meta, raw as AnyFunction);
    }
    ...
    if (meta.persist !== undefined && meta.persist !== 0) {
      ...
      arr.splice(i + 1, 0, annotate(raw as AnyFunction, { ...meta, persist: next }));
    }
...
async function processNAsync(stack: Stack, arr: unknown[], idx: number, promise: Promise<unknown>, saved: unknown[], meta: { delay?: number; persist?: number; rewind?: number; skip?: number; limit?: number }, fn: AnyFunction): Promise<Stack> {
  await promise;
  ...
  for (let i = idx + 1; i < arr.length; i++) {
    ...
    const itemMeta = getMeta(raw as AnyFunction);
    ...
    if (itemMeta.delay !== undefined && itemMeta.delay > 0) {
      arr.splice(
        i + 1,
        0,
        annotate(raw as AnyFunction, { ...itemMeta, delay: itemMeta.delay - 1 })
      );
      continue;
    }

    let localSaved: unknown[] = [];
    ...
    if (itemMeta.persist !== undefined && itemMeta.persist !== 0) {
      ...
      arr.splice(i + 1, 0, annotate(raw as AnyFunction, { ...itemMeta, persist: next }));
    }
