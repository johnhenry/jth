...
describe("h-text", () => {
  it("creates a text node from a string", () => {
    ...
    const node = s.pop();
    expect((node as any)._type).toBe(TEXT);
...
describe("h-raw", () => {
  it("creates a raw node from HTML string", () => {
    ...
    const node = s.pop();
    expect((node as any)._type).toBe(RAW);
    expect((node as any).value).toBe("<hr>");
  });
});
...
describe("h-void", () => {
  it("creates a void element", () => {
    ...
    const node = s.pop();
    expect((node as any)._type).toBe(ELEMENT);
    expect((node as any).tag).toBe("br");
...
describe("integration: composing operators", () => {
  ...
  it("builds nested elements", () => {
    ...
    const ulBlock = (ulStack: Stack) => {
      // First li
      const li1Block = (liStack: Stack) => {
        liStack.push("one");
  ...
  it("renders a full page structure", () => {
    ...
    const htmlBlock = (htmlStack: Stack) => {
      // head
      const headBlock = (headStack: Stack) => {
        headStack.push("meta");
        registry.resolve("h-void")(headStack);
        headStack.push({ charset: "utf-8" });
        registry.resolve("h-attrs")(headStack);

        const titleBlock = (titleStack: Stack) => {
      ...
      // body
      const bodyBlock = (bodyStack: Stack) => {
        const h1Block = (h1Stack: Stack) => {
          h1Stack.push("Hello");
