...
describe("E2E: Control flow", () => {
  ...
  it("if operator selects true-branch function (runtime-level)", () => {
    const stack = new Stack();
    const falseBlock = (s: Stack) => s.push("no");
    const trueBlock = (s: Stack) => s.push("yes");
  ...
  it("2-arg if executes block when truthy", () => {
    const stack = new Stack();
    stack.push(42);
    const block = (s: Stack) => s.push("matched");
    stack.push(block);
    stack.push(true);
    registry.resolve("if")(stack);
  ...
  it("if/elseif/else chain: matches first", () => {
    ...
    const b1 = (s: Stack) => s.push("first");
    stack.push(b1);
    stack.push(true);
    registry.resolve("if")(stack);

    const b2 = (s: Stack) => s.push("second");
    stack.push(b2);
    stack.push(true);
    registry.resolve("elseif")(stack);

    const b3 = (s: Stack) => s.push("fallback");
    stack.push(b3);
